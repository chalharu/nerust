...
def test_one_param_ins(base_ins, modes, val, result=None, p="", ep=None, **kwargs):
    ...
    for mode in modes:
        if mode == A_IMPLIED:
            ...
            do_test(
                "(x+)", x=1, ex=2, mem_01=val, emem_01=result
            )  # spcasm doesn't support (x)+ syntax for some reason
        elif mode == A_ABS_X:
            ...
            do_test(
                "[$ff+x]", x=2, mem_01=0x02, mem_02=0x01, mem_102=val, emem_102=result
            )
        elif mode == A_IND_DP_Y:
            do_test(
                "[$01]+y", y=5, mem_01=0xFE, mem_02=0xFF, mem_03=val, emem_03=result
            )
        elif mode == A_IMM:
...
def test_two_param_ins(base_ins, modes, val1, val2, result=None, p="", ep=None):
    ...
    for mode in modes:
        if mode == A_DP_DP:
            ...
            test_one_param_ins(
                f"{base_ins} %, #${val2:02x}", [A_DP], val1, result, p=p, ep=ep
            )
        else:
            # It's a single operand, so add a fixed A operand and treated the second one as the only operand
            test_one_param_ins(
                f"{base_ins} a, %", [mode], val2, a=val1, ea=result, p=p, ep=ep
            )


def test_16bit_ins(
    base_ins, val, result=None, ya=None, p="", eya=None, ep=None, **kwargs
):
    ...
    test(
        add_operand(base_ins, "$FF"),
        mem_1ff=val & 0xFF,
        mem_100=val >> 8,
        emem_1ff=result & 0xFF,
        emem_100=result >> 8,
        p=flags(p, add="P"),
        ep=flags(ep, p, add="P"),
        **kwargs,
...
def cbne_tests():
    for p in ["", "CZIHBVN"]:
        ...
        test_branch_taken(
            "cbne $ff+x,", p=p + "P", ep=p + "P", x=4, a=0xFF, mem_103=0x00
        )
        test_branch_not_taken(
            "cbne $ff+x,", p=p + "P", ep=p + "P", x=4, a=0xFF, mem_103=0xFF
        )
...
def clr1_tests():
    for bit in range(8):
        ...
        test_one_param_ins(
            f"clr1 %.{bit}", [A_DP], 0xFF, 0xFF - (1 << bit), p="CZIHBVN", ep="CZIHBVN"
        )
        test_one_param_ins(
            f"clr1 %.{bit}", [A_DP], 0x00, 0x00, p="CZIHBVN", ep="CZIHBVN"
        )
...
def cmp_tests():
    ...
    modes_xy = [
        ...
        A_DP,
        A_ABS,
    ]  # addressing modes for comparing with x or y registers
...
def dbnz_tests():
    ...
    test_branch_taken(
        "dbnz $02,", mem_102=0x81, emem_102=0x80, p="CZIHBPVN", ep="CZIHBPVN"
    )
...
def mov_tests():
    ...
    test_one_param_ins(
        "mov a, %", modes, 0x12, a=0x00, ea=0x12, p="CZIHBVN", ep="CIHBV"
    )
    ...
    test_one_param_ins(
        "mov %, a", modes, 0x00, a=0x12, result=0x12, p="CZIHBVN", ep="CZIHBVN"
    )
    ...
    test_one_param_ins(
        "mov x, %", modes, 0x12, x=0x00, ex=0x12, p="CZIHBVN", ep="CIHBV"
    )
    ...
    test_one_param_ins(
        "mov y, %", modes, 0x12, y=0x00, ey=0x12, p="CZIHBVN", ep="CIHBV"
    )
    ...
    test_one_param_ins(
        "mov %, x", modes, 0x00, x=0x12, result=0x12, p="CZIHBVN", ep="CZIHBVN"
    )
    ...
    test_one_param_ins(
        "mov %, y", modes, 0x00, y=0x12, result=0x12, p="CZIHBVN", ep="CZIHBVN"
    )
...
def movw_tests():
    ...
    test_16bit_ins(
        "movw %, ya", 0x0000, ya=0x1234, result=0x1234, p="CZIHBVN", ep="CZIHBVN"
    )
...
def set1_tests():
    for bit in range(8):
        ...
        test_one_param_ins(
            f"set1 %.{bit}", [A_DP], 0xFF, 0xFF, p="CZIHBVN", ep="CZIHBVN"
        )
        test_one_param_ins(
            f"set1 %.{bit}", [A_DP], 0x12, 0x12 | (1 << bit), p="CZIHBVN", ep="CZIHBVN"
        )
...
def tclr1_tests():
    ...
    test_one_param_ins(
        "tclr1", [A_ABS], 0x05, a=0x15, result=0x00, p="CZIHBPVN", ep="CIHBPV"
    )
...
def tset1_tests():
    test_one_param_ins(
        "tset1", [A_ABS], 0x11, a=0x09, result=0x19, p="CZIHBPVN", ep="CIHBPVN"
    )
    test_one_param_ins(
        "tset1", [A_ABS], 0x09, a=0x11, result=0x19, p="CZIHBPVN", ep="CIHBPV"
    )
    test_one_param_ins(
        "tset1", [A_ABS], 0x05, a=0x05, result=0x05, p="CZIHBPVN", ep="CZIHBPV"
    )
