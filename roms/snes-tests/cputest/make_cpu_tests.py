...
_ins_index_size = set(
    (
        "cpx cpy dex dey inx iny ldx ldy phx phy plx ply stx sty tax tay tsx txy tyx".split()
    )
)
# Instructions whose operand size depends on the M flag
_ins_acc_size = set(
    (
        "adc and asl bit cmp dec eor inc lda lsr ora pha pla rol ror sbc sta stz trb tsb txa tya".split()
    )
)

_re_leading_space = re.compile(r"^[ \t]+.", re.MULTILINE)

UNDOCUMENTED_COMMENT = (
    "This tests undocumented behavior - see the cputest README file for details"
)
...
def next_bank_if_needed():
    if (bank_num == 0 and tests_in_bank == TESTS_IN_BANK0) or (
        bank_num != 0 and tests_in_bank == TESTS_PER_BANK
    ):
        next_bank()
...
def test(
    ...
    **kwargs,
    ...
    inputs = (
        f"A=${a:04x} X=${x:04x} Y=${y:04x} P=${p & 0xFF:02x} E={1 if p & FLAG_E else 0}"
    )
    if s is not None:
        inputs += f" S=${s:04x}"
    if dbr is not None:
...
def test_ins_with_modes(ins, modes, val, result=None, p="", ep=None, **kwargs):
    ...
    if operand_uses_x_size:
        ...
        assert False, (
            "This function is only called on instructions with operands of variable size"
        )
    ...
    for mode in modes:
        if mode == A_IMM:
            ...
            if bits == 8:
                do_test(
                    "$FFFF", dbr=0x7E, mem=0x7EFFFF, add_flag="E"
                )  # works the same in emulation mode
        elif mode == A_ABS_X:
            ...
            if not operand_uses_x_size or bits == 8:
                do_test(
                    "$FFFF,x", x=0x1230, dbr=0x7E, mem=0x7F002F, add_flag="X"
                )  # 8 bit index
            if bits == 8:
                do_test(
                    "$FFFF,x", x=0x1230, dbr=0x7E, mem=0x7F002F, add_flag="E"
                )  # emulation mode (8 bit index)
        elif mode == A_ABS_Y:
            if not operand_uses_x_size or bits == 16:
                do_test("$FFFF,y", y=0x300, dbr=0x7E, mem=0x7F02FF)  # 16 bit index
            if not operand_uses_x_size or bits == 8:
                do_test(
                    "$FFFF,y", y=0x1230, dbr=0x7E, mem=0x7F002F, add_flag="X"
                )  # 8 bit index
            if bits == 8:
                do_test(
                    "$FFFF,y", y=0x1230, dbr=0x7E, mem=0x7F002F, add_flag="E"
                )  # emulation mode (8 bit index)
        elif mode == A_DIR:
            ...
            if not operand_uses_x_size or bits == 8:
                do_test(
                    "$02,x", x=0x1232, mem=0x33, d=0xFFFF, dbr=0x7F, add_flag="X"
                )  # 8 bit index
            if bits == 8:
            ...
            if bits == 8:
                do_test(
                    "$02,y", y=0x1232, mem=0x33, d=0xFFFF, dbr=0x7F, add_flag="X"
                )  # 8 bit index
                # E=1:  Wraps in page for DL=0, or bank for DL!=0
            ...
            assert operand_uses_m_size
            do_test(
                "($34),y", mem=0x7F0FDC, y=0x1100, d=0xFFFF, memw_0033=0xFEDC, dbr=0x7E
            )
            # 8 bit index
            do_test(
                "($34),y",
            ...
            if bits == 8:
                do_test(
                    "$7EFFFF", mem=0x7EFFFF, add_flag="E"
                )  # works the same in emulation mode
        elif mode == A_LONG_X:
            assert operand_uses_m_size
            ...
            if bits == 8:
                do_test(
                    "$7EFFFF,x", x=0x1230, mem=0x7F002F, add_flag="E"
                )  # emulation mode (8 bit index)
        elif mode == A_STACK_S:
            # currently s = 1ef
...
def adc_binary_tests():
    ...
    test_ins_with_modes(
        "adc", READ_MODES, a=0x1112, val=0xED, ea=0x1100, p="MC", ep="MCZ"
    )
    test_ins_with_modes("adc", [A_IMM], a=0x1167, val=0x20, ea=0x1187, p="M", ep="MVN")
    test_ins_with_modes(
        "adc", [A_IMM], a=0x1180, val=0x80, ea=0x1100, p="MZIVN", ep="MCZIV"
    )
...
def adc_decimal_tests():
    ...
    test_ins_with_modes(
        "adc", [A_IMM], a=0x3550, val=0x4470, ea=0x8020, p="D", ep="DNV"
    )
    test_ins_with_modes("adc", [A_IMM], a=0x4000, val=0x3999, ea=0x7999, p="D", ep="D")
    test_ins_with_modes(
        "adc", [A_IMM], a=0xDCBA, val=0xDBCA, ea=0x1EEA, p="D", ep="DC", advanced=True
    )

    test_ins_with_modes(
        "adc", READ_MODES, a=0xCC12, val=0x87, ea=0xCC00, p="DMC", ep="DMCZ"
    )
    test_ins_with_modes(
        "adc", [A_IMM], a=0xCC40, val=0x40, ea=0xCC80, p="DM", ep="DMNV"
    )
    test_ins_with_modes("adc", [A_IMM], a=0xCC40, val=0x39, ea=0xCC79, p="DM", ep="DM")
    test_ins_with_modes(
        "adc", [A_IMM], a=0xCCDC, val=0xDB, ea=0xCC1D, p="DM", ep="DMC", advanced=True
    )


def and_tests():
    test_ins_with_modes(
        "and", READ_MODES, a=0xFEFF, val=0xEF5C, p="", ea=0xEE5C, ep="N"
    )
    test_ins_with_modes("and", [A_IMM], a=0x5555, val=0xAAAA, p="", ea=0x0000, ep="Z")
    test_ins_with_modes(
        "and", [A_IMM], a=0x1234, val=0x4300, p="CZIDVN", ea=0x0200, ep="CIDV"
    )

    test_ins_with_modes(
        "and", READ_MODES, a=0x12FE, val=0xDF, p="M", ea=0x12DE, ep="MN"
    )
...
def branch_tests():
    def test_branch_taken(ins, opcode, p):
        ...
        test(
            ...
            mem_7effc0=opcode,
            mem_7effc1=127,
            mem_7effc2=OPCODE_STP,
            mem_7f0041=OPCODE_STP,
            p=p,
        )
...
def brk_cop_tests():
    ...
    for ins_name, opcode, native_addr, emul_addr in [
        ("brk", 0x00, 0x1000, 0x1008),
        ("cop #$DB", 0x02, 0x1004, 0x100C),
    ]:
...
def brl_tests():
    ...
    test(
        ...
        before_regs=before,
        after_ins=after,
        mem_7ef000=OPCODE_BRL,
        memw_7ef001=0x7FFF,
        mem_7ef003=OPCODE_STP,
        mem_7f7002=OPCODE_STP,
        p="",
    )
...
def cmp_tests():
    ...
    def do_test8(val1, val2, p, ep):
        ...
        test_ins_with_modes(
            "cpx", modes_xy, x=val1, val=val2, ex=val1 & 0xFF, p=p + "X", ep=ep + "X"
        )
        test_ins_with_modes(
            "cpy", modes_xy, y=val1, val=val2, ey=val1 & 0xFF, p=p + "X", ep=ep + "X"
        )
...
def dec_tests():
    ...
    def do_test(val, result, p, ep, bits=16):
        is8 = bits == 8
        test_ins_with_modes(
            "dec", modes, val, result, p=p + is8 * "M", ep=ep + is8 * "M"
        )
...
def eor_tests():
    test_ins_with_modes(
        "eor", READ_MODES, a=0xFEFF, val=0x6F8C, p="", ea=0x9173, ep="N"
    )
    test_ins_with_modes("eor", [A_IMM], a=0xAAAA, val=0xAAAA, p="", ea=0x0000, ep="Z")
    test_ins_with_modes(
        "eor", [A_IMM], a=0x1234, val=0x4334, p="CZIDVN", ea=0x5100, ep="CIDV"
    )

    test_ins_with_modes(
        "eor", READ_MODES, a=0x12FE, val=0x6F, p="M", ea=0x1291, ep="MN"
    )
...
def inc_tests():
    ...
    def do_test(val, result, p, ep, bits=16):
        is8 = bits == 8
        test_ins_with_modes(
            "inc", modes, val, result, p=p + is8 * "M", ep=ep + is8 * "M"
        )
...
def ld_tests():
    ...
    def do_test(val, p, ep, bits=16):
        ...
        test_ins_with_modes(
            "ldx", modes_x, val, x=0x1234, ex=val, p=p + is8 * "X", ep=ep + is8 * "X"
        )
        test_ins_with_modes(
            "ldy", modes_y, val, y=0x1234, ey=val, p=p + is8 * "X", ep=ep + is8 * "X"
        )
...
def ora_tests():
    test_ins_with_modes(
        "ora", READ_MODES, a=0x2318, val=0xB939, p="", ea=0xBB39, ep="N"
    )
    test_ins_with_modes("ora", [A_IMM], a=0x0000, val=0x0000, p="", ea=0x0000, ep="Z")
    test_ins_with_modes(
        "ora", [A_IMM], a=0x1200, val=0x4300, p="CZIDVN", ea=0x5300, ep="CIDV"
    )

    test_ins_with_modes(
        "ora", READ_MODES, a=0x1298, val=0x39, p="M", ea=0x12B9, ep="MN"
    )
...
def push_tests():
    ...
    test(
        "jml $7e8000",
        ins_name="phk",
        mem_7e8000=OPCODE_PHK,
        es=0x1EE,
        emem_1ef=0x7E,
        before_regs=jml_to_ok_instructions(0x7E8001),
        after_ins="@ok:",
    )
    test(
        "jml $7e8000",
...
def pull_misc_tests():
    ...
    test(
        "pld", memw_1f0=0x0000, es=0x1F1, d=0x9999, ed=0x0000, p="CIDVNMX", ep="CZIDVMX"
    )
    # E=1: No wrapping in page 1
...
def sbc_binary_tests():
    ...
    test_ins_with_modes(
        "sbc", [A_IMM], a=0x1234, val=0x1235, ea=0xFFFF, p="CZIV", ep="IN"
    )
    test_ins_with_modes("sbc", [A_IMM], a=0x7000, val=0xA000, ea=0xCFFF, p="", ep="NV")

    test_ins_with_modes(
        "sbc", READ_MODES, a=0xCC90, val=0x8F, ea=0xCC00, p="M", ep="MCZ"
    )
    test_ins_with_modes("sbc", [A_IMM], a=0xCC90, val=0x20, ea=0xCC70, p="MC", ep="MCV")
    test_ins_with_modes(
        "sbc", [A_IMM], a=0xCC12, val=0x13, ea=0xCCFF, p="MCZIV", ep="MIN"
    )
...
def sbc_decimal_tests():
    ...
    test_ins_with_modes(
        "sbc", [A_IMM], a=0x0000, val=0x0001, ea=0x9999, p="DC", ep="DN"
    )
    test_ins_with_modes(
        "sbc", [A_IMM], a=0x1000, val=0x9000, ea=0x2000, p="DC", ep="DV"
    )
    test_ins_with_modes("sbc", [A_IMM], a=0x1000, val=0x9001, ea=0x1999, p="DC", ep="D")
    test_ins_with_modes(
        "sbc", [A_IMM], a=0xAB1D, val=0xF1E2, ea=0x59DB, p="DC", ep="D", advanced=True
    )

    test_ins_with_modes(
        "sbc", READ_MODES, a=0xCC90, val=0x89, ea=0xCC00, p="DM", ep="DMCZ"
    )
    test_ins_with_modes(
        "sbc", [A_IMM], a=0xCC00, val=0x01, ea=0xCC99, p="DMC", ep="DMN"
    )
    test_ins_with_modes(
        "sbc", [A_IMM], a=0xCC10, val=0x90, ea=0xCC20, p="DMC", ep="DMV"
    )
    test_ins_with_modes("sbc", [A_IMM], a=0xCC10, val=0x91, ea=0xCC19, p="DMC", ep="DM")
    test_ins_with_modes(
        "sbc", [A_IMM], a=0xCCAB, val=0xF1, ea=0xCC5A, p="DMC", ep="DM", advanced=True
    )
...
def st_tests():
    ...
    def do_test(val, p, bits=16):
        ...
        test_ins_with_modes(
            "sta", modes_a, orig_val, result=val, a=val + is8 * 0x5500, p=p + is8 * "M"
        )
        test_ins_with_modes(
            "stx", modes_x, orig_val, result=val, x=val, ex=val, p=p + is8 * "X"
        )
        test_ins_with_modes(
            "sty", modes_y, orig_val, result=val, y=val, ey=val, p=p + is8 * "X"
        )
...
def trb_tests():
    modes = [A_DIR, A_ABS]
    test_ins_with_modes(
        "trb", modes, 0x9234, a=0x1630, result=0x8004, p="CZIDVN", ep="CIDVN"
    )
    test_ins_with_modes("trb", modes, 0xAAAA, a=0x5555, result=0xAAAA, p="", ep="Z")

    test_ins_with_modes(
        "trb", modes, 0x92, a=0x16, result=0x80, p="CZIDMVN", ep="CIDMVN"
    )
...
def tsb_tests():
    modes = [A_DIR, A_ABS]
    test_ins_with_modes(
        "tsb", modes, 0x9234, a=0x1630, result=0x9634, p="CZIDVN", ep="CIDVN"
    )
    test_ins_with_modes("tsb", modes, 0xAAAA, a=0x5555, result=0xFFFF, p="", ep="Z")

    test_ins_with_modes(
        "tsb", modes, 0x92, a=0x16, result=0x96, p="CZIDMVN", ep="CIDMVN"
    )
...
def transfer_axy_tests():
    ...
    test(
        "tax", a=0x1234, x=0x5678, ex=0x1234, p="CZIDMVN", ep="CIDMV"
    )  # 8 bit A - transfers 16 bits
    ...
    test(
        "tay", a=0x1234, y=0x5678, ey=0x1234, p="CZIDMVN", ep="CIDMV"
    )  # 8 bit A - transfers 16 bits
    ...
    test(
        "txa", x=0x00CD, a=0x1234, ea=0xCD, p="CZIDXVN", ep="CIDXV"
    )  # 8 bit X - transfers 16 bits
    test(
        "txa", x=0xABCD, a=0x1234, ea=0x12CD, p="M", ep="MN"
    )  # 8 bit A - transfers 8 bits
    test(
        "txa", x=0xAB00, a=0x1234, ea=0x1200, p="M", ep="MZ"
    )  # 8 bit A - transfers 8 bits

    test("tya", y=0x9876, a=0x1234, ea=0x9876, p="", ep="N")
    test("tya", y=0x0000, a=0x1234, ea=0x0000, p="", ep="Z")
    test(
        "tya", y=0x00CD, a=0x1234, ea=0xCD, p="CZIDXVN", ep="CIDXV"
    )  # 8 bit Y - transfers 16 bits
    test(
        "tya", y=0xABCD, a=0x1234, ea=0x12CD, p="M", ep="MN"
    )  # 8 bit A - transfers 8 bits
    test(
        "tya", y=0xAB00, a=0x1234, ea=0x1200, p="M", ep="MZ"
    )  # 8 bit A - transfers 8 bits
...
def additional_tests():
    ...
    after = f"""\
        ...
        @ok:"""
    before = (
        jml_to_ok_instructions(0x7E0000)
        + "\n"
        + jml_to_label_instructions(0x7F0000, "@not_ok")
    )
    test(
        "jml $7EFFFF",
        ins_name="nop  ; test PC wrapping from $FFFF",
        before_regs=before,
        after_ins=after,
