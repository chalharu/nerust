...
def is_small_tile_not_transparent(image, transparent_color, xpos, ypos):
    ...
    return any(
        convert_rgb_color(image.getpixel((x, y))) != transparent_color
...
def extract_small_tile(image, xpos, ypos):
    ...
    return [
        convert_rgb_color(image.getpixel((x, y)))
        for y in range(ypos, ypos + 8)
        for x in range(xpos, xpos + 8)
    ]
...
_H_FLIP_ORDER_SMALL = [
    (y * 8 + x) for y, x in itertools.product(range(8), reversed(range(8)))
]
_V_FLIP_ORDER_SMALL = [
    (y * 8 + x) for y, x in itertools.product(reversed(range(8)), range(8))
]
...
_H_FLIP_ORDER_LARGE = [
    (y * 16 + x) for y, x in itertools.product(range(16), reversed(range(16)))
]
_V_FLIP_ORDER_LARGE = [
    (y * 16 + x) for y, x in itertools.product(reversed(range(16)), range(16))
]
...
def image_to_snes(image, palette_image, bpp):
    ...
    tilemap, tileset = convert_tilemap_and_tileset(
        extract_tilemap_tiles(image), create_palettes_map(palette_image, bpp)
    )
