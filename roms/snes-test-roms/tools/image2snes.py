...
def parse_arguments():
    ...
    parser.add_argument(
        "-f", "--format", required=True, choices=FORMATS_BPP.keys(), help="tile format"
    )
    parser.add_argument(
        "-t", "--tileset-output", required=True, help="tileset output file"
    )
    parser.add_argument(
        "-m", "--tilemap-output", required=True, help="tilemap output file"
    )
    parser.add_argument(
        "-p", "--palette-output", required=True, help="palette output file"
    )
    parser.add_argument(
        "--high-priority",
        required=False,
    ...
    parser.add_argument("image_filename", action="store", help="Indexed png image")
