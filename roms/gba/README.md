...
| `alyosha_gba-tests/` | https://github.com/alyosha-tas/gba-tests | MIT |
...
- `alyosha_gba-tests`: fork of jsmolka `gba-tests` with extra Bus/DMA/Interactions/LDM/Serial/fifo_dma/irq/prefetcher/psr/timer/timing suites, merged from commit `66f5f1d6` (master, alyosha-tas/gba-tests) under `alyosha_gba-tests/` (text files renormalized to LF)
...
- jsmolka `gba-tests` ROMs are assembled with [FASMARM](https://arm.flatassembler.net/). The repository includes prebuilt `.gba` files; rebuild with `fasmarm` if needed. The same applies to the `alyosha_gba-tests` fork (prebuilt `.gba` files included; `unsafe/` ROMs are excluded from the matrix like upstream).
...
- `alyosha_gba-tests` ROMs follow the same `R12`-first-failure convention (all standalone ROMs end in `m_test_eval r12`; `0` means pass). The matrix registers every prebuilt `.gba` except `unsafe/` via a glob pattern with `R12 == 0` verification.
