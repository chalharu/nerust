...
Test ROMs are executed via the `nerust_gba_rom_test` crate (`gba/rom_test/`). The manifest `gba/rom_test/rom_tests.yaml` defines `rom_root: ../../roms/gba` and suites `jsmolka_gba-tests`, `nba-emu_hw-test` and `armwrestler-gba-fixed`. Run with:
