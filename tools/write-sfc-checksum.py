...
def calculate_checksum(rom_data, bank_size, header_offset, expected_map_mode):
    ...
    if rom_size % bank_size != 0:
        raise RuntimeError(
            f"sfc file is an invalid size (expected a multiple of {bank_size // 1024} KiB)."
        )
    ...
    if not check_header_exists(rom_data, header_offset, expected_map_mode):
        raise RuntimeError(
            "Could not find header.  Header must match `snes_header.inc` (and the checksum bytes MUST be unmodified).  Is the --hirom/--lorom argument correct?"
        )
...
def write_sfc_checksum(sfc_filename, bank_size, header_offset, expected_map_mode):
    ...
    with open(sfc_filename, "r+b") as fp:
        ...
        if fp.read(1):
            raise RuntimeError(
                f"sfc file is too large (max {MAX_ROM_SIZE // 1024} KiB)."
            )

        checksum_bytes = calculate_checksum(
            rom_data, bank_size, header_offset, expected_map_mode
        )
...
def parse_arguments():
    ...
    parser.add_argument(
        "sfc_filename", action="store", help="sfc file (MODIFIED IN PLACE)"
    )
