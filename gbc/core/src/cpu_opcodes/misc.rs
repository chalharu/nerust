...
impl CpuStepState for Nop {
    fn exec(_: &mut Lr35902Cpu, _: &mut GbcMemoryBus, step: u8) -> StepResult {
        if step == 0 { return StepResult::Exit; }
        StepResult::Continue
    }
}
...
impl CpuStepState for Rlca {
    fn exec(core: &mut Lr35902Cpu, _: &mut GbcMemoryBus, step: u8) -> StepResult {
        if step == 0 { let c = core.registers.a & 0x80 != 0; core.registers.a = (core.registers.a << 1) | c as u8; core.registers.set_z(false); core.registers.set_n(false); core.registers.set_h(false); core.registers.set_c(c); return StepResult::Exit; }
        StepResult::Continue
    }
}
...
impl CpuStepState for Rla {
    fn exec(core: &mut Lr35902Cpu, _: &mut GbcMemoryBus, step: u8) -> StepResult {
        if step == 0 { let c = core.registers.a & 0x80 != 0; core.registers.a = (core.registers.a << 1) | core.registers.c_flag() as u8; core.registers.set_z(false); core.registers.set_n(false); core.registers.set_h(false); core.registers.set_c(c); return StepResult::Exit; }
        StepResult::Continue
    }
}
...
impl CpuStepState for Rrca {
    fn exec(core: &mut Lr35902Cpu, _: &mut GbcMemoryBus, step: u8) -> StepResult {
        if step == 0 { let c = core.registers.a & 0x01 != 0; core.registers.a = (core.registers.a >> 1) | (if c { 0x80 } else { 0 }); core.registers.set_z(false); core.registers.set_n(false); core.registers.set_h(false); core.registers.set_c(c); return StepResult::Exit; }
        StepResult::Continue
    }
}
...
impl CpuStepState for Rra {
    fn exec(core: &mut Lr35902Cpu, _: &mut GbcMemoryBus, step: u8) -> StepResult {
        if step == 0 { let c = core.registers.a & 0x01 != 0; core.registers.a = (core.registers.a >> 1) | (if core.registers.c_flag() { 0x80 } else { 0 }); core.registers.set_z(false); core.registers.set_n(false); core.registers.set_h(false); core.registers.set_c(c); return StepResult::Exit; }
        StepResult::Continue
    }
}
...
impl CpuStepState for Daa {
    fn exec(core: &mut Lr35902Cpu, _: &mut GbcMemoryBus, step: u8) -> StepResult {
        if step == 0 {
            ...
            if !n {
                if c || a > 0x99 { a = a.wrapping_add(0x60); core.registers.set_c(true); }
                if h || (a & 0x0F) > 0x09 { a = a.wrapping_add(0x06); }
            } else {
                if c { a = a.wrapping_sub(0x60); }
                if h { a = a.wrapping_sub(0x06); }
            }
            core.registers.set_z(a == 0); core.registers.set_h(false); core.registers.a = a;
...
impl CpuStepState for Cpl {
    fn exec(core: &mut Lr35902Cpu, _: &mut GbcMemoryBus, step: u8) -> StepResult {
        if step == 0 { core.registers.a = !core.registers.a; core.registers.set_n(true); core.registers.set_h(true); return StepResult::Exit; }
        StepResult::Continue
    }
}
...
impl CpuStepState for Scf {
    fn exec(core: &mut Lr35902Cpu, _: &mut GbcMemoryBus, step: u8) -> StepResult {
        if step == 0 { core.registers.set_n(false); core.registers.set_h(false); core.registers.set_c(true); return StepResult::Exit; }
        StepResult::Continue
    }
}
...
impl CpuStepState for Ccf {
    fn exec(core: &mut Lr35902Cpu, _: &mut GbcMemoryBus, step: u8) -> StepResult {
        if step == 0 { let c = core.registers.c_flag(); core.registers.set_n(false); core.registers.set_h(false); core.registers.set_c(!c); return StepResult::Exit; }
        StepResult::Continue
    }
}
...
impl CpuStepState for Halt {
    fn exec(_: &mut Lr35902Cpu, bus: &mut GbcMemoryBus, step: u8) -> StepResult {
        if step == 0 { bus.halt_cpu(); return StepResult::Exit; }
        StepResult::Continue
    }
}
...
impl CpuStepState for Stop {
    fn exec(_: &mut Lr35902Cpu, bus: &mut GbcMemoryBus, step: u8) -> StepResult {
        if step == 0 { bus.stop(); return StepResult::Exit; }
        StepResult::Continue
    }
}
pub(crate) struct Ei;
impl CpuStepState for Ei {
    fn exec(core: &mut Lr35902Cpu, _: &mut GbcMemoryBus, step: u8) -> StepResult {
        if step == 0 { core.ime_delayed = true; return StepResult::Exit; }
        StepResult::Continue
    }
}
pub(crate) struct Di;
impl CpuStepState for Di {
    fn exec(_: &mut Lr35902Cpu, bus: &mut GbcMemoryBus, step: u8) -> StepResult {
        if step == 0 { bus.set_ime(false); return StepResult::Exit; }
        StepResult::Continue
    }
}
...
impl<const STEP: u8, const M: u8> CpuStepState for InvalidOp<STEP, M> {
    fn exec(core: &mut Lr35902Cpu, bus: &mut GbcMemoryBus, step: u8) -> StepResult {
        ...
        core.pc_read(bus);
        if step >= M { return StepResult::Exit; }
        StepResult::Continue
    }
}
