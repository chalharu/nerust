...
impl<const R: u8> CpuStepState for IncR8<R> {
    fn exec(core: &mut Lr35902Cpu, _: &mut GbcMemoryBus, step: u8) -> StepResult {
        if step == 0 { let v = read_r8(core, R); let r = v.wrapping_add(1); write_r8(core, R, r); core.registers.set_h((v & 0x0F) == 0x0F); core.registers.set_z(r == 0); core.registers.set_n(false); return StepResult::Exit; }
        StepResult::Continue
    }
}
...
impl<const R: u8> CpuStepState for DecR8<R> {
    fn exec(core: &mut Lr35902Cpu, _: &mut GbcMemoryBus, step: u8) -> StepResult {
        if step == 0 { let v = read_r8(core, R); let r = v.wrapping_sub(1); write_r8(core, R, r); core.registers.set_h((v & 0x0F) == 0); core.registers.set_z(r == 0); core.registers.set_n(true); return StepResult::Exit; }
        StepResult::Continue
    }
}
...
impl CpuStepState for IncHlIndirect {
    fn exec(core: &mut Lr35902Cpu, bus: &mut GbcMemoryBus, step: u8) -> StepResult {
        if step == 0 { return StepResult::Continue; }
        if step == 1 { core.operands[0] = bus.read(core.registers.hl()); return StepResult::Continue; }
        if step == 2 { let v = core.operands[0]; let r = v.wrapping_add(1); bus.write(core.registers.hl(), r); core.registers.set_h((v & 0x0F) == 0x0F); core.registers.set_z(r == 0); core.registers.set_n(false); return StepResult::Exit; }
        unreachable!()
    }
}
...
impl CpuStepState for DecHlIndirect {
    fn exec(core: &mut Lr35902Cpu, bus: &mut GbcMemoryBus, step: u8) -> StepResult {
        if step == 0 { return StepResult::Continue; }
        if step == 1 { core.operands[0] = bus.read(core.registers.hl()); return StepResult::Continue; }
        if step == 2 { let v = core.operands[0]; let r = v.wrapping_sub(1); bus.write(core.registers.hl(), r); core.registers.set_h((v & 0x0F) == 0); core.registers.set_z(r == 0); core.registers.set_n(true); return StepResult::Exit; }
        unreachable!()
    }
}
