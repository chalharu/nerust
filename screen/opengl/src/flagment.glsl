...
mediump float texel_u8(vec4 texel, int channel) {
    return floor(clamp(texel[channel], 0.0, 1.0) * 255.0 + 0.5);
}

mediump int decode_u16(mediump float high, mediump float low) {
...
mediump vec3 ntsc_color(int output_x, int output_y) {
    ...
    mediump float row3 = float(phase * NTSC_ENTRY_STRIDE + ((sample + 7) % 14));
    mediump float row4 = float(phase * NTSC_ENTRY_STRIDE + ((sample + 5) % 7 + 21));
    mediump float row5 = float(phase * NTSC_ENTRY_STRIDE + ((sample + 3) % 7 + 35));
