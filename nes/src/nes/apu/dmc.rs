...
impl DMC {
    ...
    pub fn step_timer(&mut self, interrupt: &mut Interrupt, cartridge: &mut Box<Cartridge>) {
    ...
    pub fn step_reader(&mut self, interrupt: &mut Interrupt, _cartridge: &mut Box<Cartridge>) {
        if self.bit_count == 0 {
            self.bit_count = 8;
            if self.need_buffer {
                self.enabled = false;
