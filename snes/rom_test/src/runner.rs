...
fn assertion_failures(case: &RomCase, core: &Core) -> Result<Vec<String>, ManifestError> {
    case.assertions
        ...
        .map(|assertion| match assertion {
            Assertion::BusU8 { .. } => evaluate_bus_u8_assertion(assertion, core),
            Assertion::BusU16 { .. } => evaluate_bus_u16_assertion(assertion, core),
        })
        .filter_map(Result::transpose)
        .collect()
}

fn evaluate_bus_u8_assertion(
    assertion: &Assertion,
    core: &Core,
) -> Result<Option<String>, ManifestError> {
    let address = assertion.address()?;
    let expected = assertion.expected_u8()?;
    let actual = core.peek(address);

    if actual == expected {
        Ok(None)
    } else {
        Ok(Some(format!(
            "bus_u8  @ 0x{address:06X}: expected 0x{expected:02X}, got 0x{actual:02X}"
        )))
    }
}

fn evaluate_bus_u16_assertion(
    assertion: &Assertion,
    core: &Core,
) -> Result<Option<String>, ManifestError> {
    let address = assertion.address()?;
    let expected = assertion.expected_u16()?;
    let actual = u16::from_le_bytes([core.peek(address), core.peek(address + 1)]);

    if actual == expected {
        Ok(None)
    } else {
        Ok(Some(format!(
            "bus_u16 @ 0x{address:06X}: expected 0x{expected:04X}, got 0x{actual:04X}"
        )))
    }
}
