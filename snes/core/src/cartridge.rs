...
mod tests {
    ...
    const GSU_CONDITIONAL_BRANCH_PROGRAM: &[u8] = &[
        0x05, 0x07, 0xF1, 0x00, 0x03, 0xF0, 0xAD, 0xDE, 0x31, 0xF1, 0x00, 0x03, 0xF0, 0x34, 0x12,
        0x31, 0xF0, 0x00, 0x00, 0x3E, 0x61, 0x0B, 0x07, 0xF1, 0x02, 0x03, 0xF0, 0xAD, 0xDE, 0x31,
        0xF1, 0x02, 0x03, 0xF0, 0xFE, 0xCA, 0x31, 0xF0, 0x02, 0x00, 0x3E, 0x61, 0x0D, 0x07, 0xF1,
        0x04, 0x03, 0xF0, 0xAD, 0xDE, 0x31, 0xF1, 0x04, 0x03, 0xF0, 0xEF, 0xBE, 0x31, 0xF0, 0x00,
        0x00, 0x3E, 0x61, 0x00, 0x01,
    ];
    const GSU_DIRECT_RAM_PROGRAM: &[u8] = &[
        0xF2, 0x33, 0x22, 0x3E, 0xF2, 0x00, 0x03, 0x3D, 0xF3, 0x00, 0x03, 0x3E, 0xF3, 0x10, 0x03,
        0xF2, 0x55, 0x44, 0x3E, 0xF2, 0x01, 0x03, 0x3D, 0xF4, 0x01, 0x03, 0xF4, 0x77, 0x66, 0x90,
        0x3D, 0xF5, 0x01, 0x03, 0x3E, 0xF5, 0x20, 0x03, 0x00, 0x01,
    ];
    const GSU_SOURCE_STORE_PROGRAM: [u8; 19] = [
        0xF1, 0xEF, 0xBE, 0xB1, 0xF2, 0x00, 0x03, 0x32, 0xF1, 0xAB, 0x12, 0xB1, 0xF2, 0x02, 0x03,
        0x3D, 0x32, 0x00, 0x01,
    ];
    const GSU_ALU_EXTENSIONS_PROGRAM: &[u8] = &[
        0x91, 0xF0, 0x02, 0x00, 0x3E, 0x61, 0xF0, 0xFF, 0xFF, 0x3F, 0x51, 0xF1, 0x00, 0x03, 0x31,
        0xF0, 0x00, 0x00, 0x3E, 0x61, 0xF0, 0x03, 0x00, 0xF1, 0x01, 0x00, 0x3D, 0x61, 0xF1, 0x02,
        0x03, 0x31, 0xF0, 0xFF, 0x00, 0x3F, 0x7F, 0xF1, 0x04, 0x03, 0x31, 0xF0, 0x01, 0x80, 0x04,
        0xF1, 0x06, 0x03, 0x31, 0x97, 0xF1, 0x08, 0x03, 0x31, 0xF0, 0xFE, 0x00, 0xF1, 0x02, 0x00,
        0x3D, 0x81, 0xF1, 0x0A, 0x03, 0x31, 0xF7, 0xCD, 0xAB, 0xF8, 0x12, 0xEF, 0x70, 0xF1, 0x0C,
    ...
    const GSU_CACHE_PROGRAM: &[u8] = &[0x02, 0x00, 0x01];
    const GSU_LJMP_SOURCE_PROGRAM: &[u8] = &[
        0xF0, 0x00, 0x80, 0xF8, 0x01, 0x00, 0xB0, 0x3D, 0x98, 0x00, 0x01,
    ];
    const GSU_LJMP_TARGET_PROGRAM: &[u8] = &[0xF1, 0x00, 0x04, 0xF0, 0xEF, 0xBE, 0x31, 0x00, 0x01];
    ...
    const GSU_RAM_LOAD_FLAGS_PROGRAM: [u8; 5] = [0xF1, 0x40, 0x01, 0x41, 0x00];
    ...
    #[test]
    fn super_fx_move_copies_between_registers_without_with_prefix() {
        let mut cartridge = Cartridge::from_bytes(&build_hirom_with_header(
            "HIROM GSU MOVE",
            0x31,
            0x15,
        ...
        for (offset, value) in GSU_MOVE_REGISTER_PROGRAM.iter().copied().enumerate() {
            assert!(cartridge.write(0x700080 + offset as u32, value));
        }
        start_super_fx_program(&mut cartridge, 0x70, 0x0080);
    ...
    #[test]
    fn super_fx_ram_loads_preserve_alu_flags() {
        let mut cartridge = Cartridge::from_bytes(&build_hirom_with_header(
            "HIROM GSU RAM FLAGS",
            0x31,
            0x15,
        ...
        for (offset, value) in GSU_RAM_LOAD_FLAGS_PROGRAM.iter().copied().enumerate() {
            assert!(cartridge.write(0x700080 + offset as u32, value));
        }
        assert!(cartridge.write(0x003030, 0x08));
        start_super_fx_program(&mut cartridge, 0x70, 0x0080);
    ...
    #[test]
    fn super_fx_cache_and_long_jump_update_cbr_and_pbr() {
        let mut rom = build_hirom_with_header("HIROM GSU CACHE", 0x31, 0x15, None, 0x0C);
        rom[..GSU_CACHE_PROGRAM.len()].copy_from_slice(GSU_CACHE_PROGRAM);
        let mut cartridge = Cartridge::from_bytes(&rom).unwrap();
        ...
        rom[0x8000..0x8000 + GSU_LJMP_TARGET_PROGRAM.len()]
            .copy_from_slice(GSU_LJMP_TARGET_PROGRAM);
        let mut cartridge = Cartridge::from_bytes(&rom).unwrap();

        start_super_fx_program(&mut cartridge, 0x00, 0x8000);

        assert_eq!(cartridge.read(0x003034), Some(0x01));
        assert_eq!(cartridge.read(0x00303E), Some(0x00));
        assert_eq!(cartridge.read(0x00303F), Some(0x80));
        assert_eq!(cartridge.read(0x700400), Some(0xEF));
        assert_eq!(cartridge.read(0x700401), Some(0xBE));
    }
    ...
    #[test]
    fn super_fx_conditional_branches_use_alu_flags() {
        let mut cartridge = Cartridge::from_bytes(&build_hirom_with_header(
            "HIROM GSU BRANCHES",
            0x31,
            0x15,
        ...
        for (offset, value) in GSU_CONDITIONAL_BRANCH_PROGRAM.iter().copied().enumerate() {
            assert!(cartridge.write(0x700080 + offset as u32, value));
        }
        start_super_fx_program(&mut cartridge, 0x70, 0x0080);
    ...
    #[test]
    fn super_fx_direct_ram_ops_and_sbk_use_last_address() {
        let mut cartridge = Cartridge::from_bytes(&build_hirom_with_header(
            "HIROM GSU DIRECT RAM",
            0x31,
            0x15,
            None,
            0x0C,
        ...
        for (offset, value) in GSU_DIRECT_RAM_PROGRAM.iter().copied().enumerate() {
            assert!(cartridge.write(0x700080 + offset as u32, value));
        }
        start_super_fx_program(&mut cartridge, 0x70, 0x0080);

        assert_eq!(cartridge.read(0x700310), Some(0x33));
        assert_eq!(cartridge.read(0x700311), Some(0x22));
        assert_eq!(cartridge.read(0x003006), Some(0x33));
        assert_eq!(cartridge.read(0x003007), Some(0x22));
        assert_eq!(cartridge.read(0x700300), Some(0x66));
        assert_eq!(cartridge.read(0x700301), Some(0x77));
        assert_eq!(cartridge.read(0x003008), Some(0x77));
        assert_eq!(cartridge.read(0x003009), Some(0x66));
        assert_eq!(cartridge.read(0x00300A), Some(0x77));
        assert_eq!(cartridge.read(0x00300B), Some(0x66));
        assert_eq!(cartridge.read(0x700320), Some(0x77));
        assert_eq!(cartridge.read(0x700321), Some(0x66));
    }
    ...
    fn super_fx_direct_stores_use_selected_source_register() {
        ...
        for (offset, value) in GSU_SOURCE_STORE_PROGRAM.iter().copied().enumerate() {
            assert!(cartridge.write(0x700080 + offset as u32, value));
        }
        start_super_fx_program(&mut cartridge, 0x70, 0x0080);

        assert_eq!(cartridge.read(0x700300), Some(0xEF));
        assert_eq!(cartridge.read(0x700301), Some(0xBE));
        assert_eq!(cartridge.read(0x700302), Some(0xAB));
    }

    #[test]
    fn super_fx_extended_alu_ops_update_results() {
        let mut cartridge = Cartridge::from_bytes(&build_hirom_with_header(
            "HIROM GSU ALU EXT",
            0x31,
            0x15,
        ...
        for (offset, value) in GSU_ALU_EXTENSIONS_PROGRAM.iter().copied().enumerate() {
            assert!(cartridge.write(0x700080 + offset as u32, value));
        }
        start_super_fx_program(&mut cartridge, 0x70, 0x0080);

        assert_eq!(cartridge.read(0x700300), Some(0x01));
        assert_eq!(cartridge.read(0x700301), Some(0x00));
        assert_eq!(cartridge.read(0x700302), Some(0x01));
        assert_eq!(cartridge.read(0x700303), Some(0x00));
        assert_eq!(cartridge.read(0x700304), Some(0xF0));
        assert_eq!(cartridge.read(0x700305), Some(0x00));
        assert_eq!(cartridge.read(0x700306), Some(0x02));
        assert_eq!(cartridge.read(0x700307), Some(0x00));
        assert_eq!(cartridge.read(0x700308), Some(0x01));
        assert_eq!(cartridge.read(0x700309), Some(0x80));
        assert_eq!(cartridge.read(0x70030A), Some(0xFC));
        assert_eq!(cartridge.read(0x70030B), Some(0x01));
        assert_eq!(cartridge.read(0x70030C), Some(0xEF));
        assert_eq!(cartridge.read(0x70030D), Some(0xAB));
        assert_eq!(cartridge.read(0x70030E), Some(0x82));
        assert_eq!(cartridge.read(0x70030F), Some(0x00));
        assert_eq!(cartridge.read(0x700310), Some(0x00));
        assert_eq!(cartridge.read(0x700311), Some(0x10));
        assert_eq!(cartridge.read(0x700312), Some(0x00));
    ...
    fn dsp1_lorom_executes_tier1_commands() {
        ...
        assert!(cartridge.write(0x308000, 0x00));
        assert_eq!(cartridge.read(0x30C000), Some(0x80));
        write_dsp1_word(&mut cartridge, 0x308000, 0x4000);
        write_dsp1_word(&mut cartridge, 0x308000, 0x4000);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x2000);
        assert_eq!(cartridge.read(0x30C000), Some(0x84));

        assert!(cartridge.write(0x308000, 0x20));
        write_dsp1_word(&mut cartridge, 0x308000, 0x4000);
        write_dsp1_word(&mut cartridge, 0x308000, 0x4000);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x2001);

        assert!(cartridge.write(0x308000, 0x0F));
        write_dsp1_word(&mut cartridge, 0x308000, 0xFFFF);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x0000);

        assert!(cartridge.write(0x308000, 0x2F));
        write_dsp1_word(&mut cartridge, 0x308000, 0xFFFF);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x0100);

        assert!(cartridge.write(0x308000, 0x08));
        write_dsp1_word(&mut cartridge, 0x308000, 3);
        write_dsp1_word(&mut cartridge, 0x308000, 4);
        write_dsp1_word(&mut cartridge, 0x308000, 12);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 169);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0);

        assert!(cartridge.write(0x308000, 0x18));
        write_dsp1_word(&mut cartridge, 0x308000, 3);
        write_dsp1_word(&mut cartridge, 0x308000, 4);
        write_dsp1_word(&mut cartridge, 0x308000, 12);
        write_dsp1_word(&mut cartridge, 0x308000, 13);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0);

        assert!(cartridge.write(0x308000, 0x38));
        write_dsp1_word(&mut cartridge, 0x308000, 3);
        write_dsp1_word(&mut cartridge, 0x308000, 4);
        write_dsp1_word(&mut cartridge, 0x308000, 12);
        write_dsp1_word(&mut cartridge, 0x308000, 13);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 1);

        assert!(cartridge.write(0x308000, 0x10));
        write_dsp1_word(&mut cartridge, 0x308000, 0x4000);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x7FFF);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 1);

        assert!(cartridge.write(0x308000, 0x30));
        write_dsp1_word(&mut cartridge, 0x308000, 0x2000);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x7FFF);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 2);

        assert!(cartridge.write(0x308000, 0x10));
        write_dsp1_word(&mut cartridge, 0x308000, (-0x4000_i16) as u16);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        assert_eq!(
            read_dsp1_word(&mut cartridge, 0x308000),
            (-0x4000_i16) as u16
        );
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 2);
        ...
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x7FFF);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x002F);
    }
    ...
    fn dsp1_lorom_executes_geometry_commands() {
        ...
        assert!(cartridge.write(0x308000, 0x0C));
        write_dsp1_word(&mut cartridge, 0x308000, 0x4000);
        write_dsp1_word(&mut cartridge, 0x308000, 10);
        write_dsp1_word(&mut cartridge, 0x308000, 20);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 20);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), (-10_i16) as u16);

        assert!(cartridge.write(0x308000, 0x1C));
        write_dsp1_word(&mut cartridge, 0x308000, 0x4000);
        write_dsp1_word(&mut cartridge, 0x308000, 0x0000);
        write_dsp1_word(&mut cartridge, 0x308000, 0x0000);
        write_dsp1_word(&mut cartridge, 0x308000, 10);
        write_dsp1_word(&mut cartridge, 0x308000, 20);
        write_dsp1_word(&mut cartridge, 0x308000, 30);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 20);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), (-10_i16) as u16);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 30);

        assert!(cartridge.write(0x308000, 0x14));
        write_dsp1_word(&mut cartridge, 0x308000, 100);
        write_dsp1_word(&mut cartridge, 0x308000, 200);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        write_dsp1_word(&mut cartridge, 0x308000, 5);
        write_dsp1_word(&mut cartridge, 0x308000, 7);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 100);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 205);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 7);

        assert!(cartridge.write(0x308000, 0x34));
        write_dsp1_word(&mut cartridge, 0x308000, 100);
        write_dsp1_word(&mut cartridge, 0x308000, 200);
        write_dsp1_word(&mut cartridge, 0x308000, 0x4000);
        write_dsp1_word(&mut cartridge, 0x308000, 10);
        write_dsp1_word(&mut cartridge, 0x308000, 20);
        write_dsp1_word(&mut cartridge, 0x308000, 30);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 80);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 210);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x4000 + 30);

        assert!(cartridge.write(0x308000, 0x28));
        write_dsp1_word(&mut cartridge, 0x308000, 3);
        write_dsp1_word(&mut cartridge, 0x308000, 4);
        write_dsp1_word(&mut cartridge, 0x308000, 12);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 13);
    }
    ...
    fn dsp1_lorom_executes_projection_commands() {
        ...
        write_dsp1_word(&mut cartridge, 0x308000, 100);
        write_dsp1_word(&mut cartridge, 0x308000, 200);
        write_dsp1_word(&mut cartridge, 0x308000, 300);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        write_dsp1_word(&mut cartridge, 0x308000, 0x4000);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        ...
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x4000);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 100);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 200);
        ...
        assert!(cartridge.write(0x308000, 0x1A));
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x0100);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0);
        ...
        assert!(cartridge.write(0x308000, 0x06));
        write_dsp1_word(&mut cartridge, 0x308000, 116);
        write_dsp1_word(&mut cartridge, 0x308000, 232);
        write_dsp1_word(&mut cartridge, 0x308000, 300);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 16);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 32);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x0100);

        assert!(cartridge.write(0x308000, 0x16));
        write_dsp1_word(&mut cartridge, 0x308000, 101);
        write_dsp1_word(&mut cartridge, 0x308000, 200);
        write_dsp1_word(&mut cartridge, 0x308000, (-20000_i16) as u16);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x4000);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x7FFF);

        assert!(cartridge.write(0x308000, 0x3E));
        write_dsp1_word(&mut cartridge, 0x308000, 16);
        write_dsp1_word(&mut cartridge, 0x308000, 32);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 116);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 232);
    }
    ...
    fn dsp1_lorom_executes_matrix_commands() {
        ...
        assert!(cartridge.write(0x308000, 0x01));
        write_dsp1_word(&mut cartridge, 0x308000, 0x4000);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        write_dsp1_word(&mut cartridge, 0x308000, 0);

        assert!(cartridge.write(0x308000, 0x0D));
        write_dsp1_word(&mut cartridge, 0x308000, 40);
        write_dsp1_word(&mut cartridge, 0x308000, (-80_i16) as u16);
        write_dsp1_word(&mut cartridge, 0x308000, 120);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 10);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), (-20_i16) as u16);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 30);

        assert!(cartridge.write(0x308000, 0x03));
        write_dsp1_word(&mut cartridge, 0x308000, 40);
        write_dsp1_word(&mut cartridge, 0x308000, (-80_i16) as u16);
        write_dsp1_word(&mut cartridge, 0x308000, 120);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 10);
        ...
        assert!(cartridge.write(0x308000, 0x0B));
        write_dsp1_word(&mut cartridge, 0x308000, 40);
        write_dsp1_word(&mut cartridge, 0x308000, (-80_i16) as u16);
        write_dsp1_word(&mut cartridge, 0x308000, 120);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 10);

        assert!(cartridge.write(0x308000, 0x11));
        write_dsp1_word(&mut cartridge, 0x308000, 0x4000);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        assert!(cartridge.write(0x308000, 0x1D));
        write_dsp1_word(&mut cartridge, 0x308000, 64);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 16);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0);
        ...
        assert!(cartridge.write(0x308000, 0x21));
        write_dsp1_word(&mut cartridge, 0x308000, 0x4000);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        assert!(cartridge.write(0x308000, 0x2D));
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        write_dsp1_word(&mut cartridge, 0x308000, 64);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0);
        ...
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        assert!(cartridge.write(0x308000, 0x09));
        write_dsp1_word(&mut cartridge, 0x308000, 4);
        write_dsp1_word(&mut cartridge, 0x308000, 4);
        write_dsp1_word(&mut cartridge, 0x308000, 0);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0);
    ...
    fn dsp1_lorom_executes_memory_command_aliases() {
        ...
        assert!(cartridge.write(0x308000, 0x07));
        write_dsp1_word(&mut cartridge, 0x308000, 0xFFFF);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x0000);

        let mut cartridge = new_cartridge();
        assert!(cartridge.write(0x308000, 0x27));
        write_dsp1_word(&mut cartridge, 0x308000, 0xFFFF);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x0100);

        let mut cartridge = new_cartridge();
        assert!(cartridge.write(0x308000, 0x17));
        write_dsp1_word(&mut cartridge, 0x308000, 0xFFFF);
        for _ in 0..1023 {
            assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x0000);
            assert_eq!(cartridge.read(0x30C000), Some(0x80));
        ...
        let mut cartridge = new_cartridge();
        assert!(cartridge.write(0x308000, 0x37));
        write_dsp1_word(&mut cartridge, 0x308000, 0xFFFF);
        assert_eq!(read_dsp1_word(&mut cartridge, 0x308000), 0x0000);
        assert_eq!(cartridge.read(0x30C000), Some(0x80));
    }
    ...
    fn dsp1_hirom_register_window_reports_ready_status() {
        ...
        assert_eq!(cartridge.read(0x807000), Some(0x84));
        assert_eq!(cartridge.read(0xC08000), Some(0xEA));
    }
    ...
    fn dsp1_register_window_supports_documented_board_maps() {
        ...
        assert!(cartridge.write(0x206000, 0x66));
        assert_eq!(cartridge.read(0x206000), Some(0x66));
        assert_eq!(cartridge.read(0x207000), Some(0x84));
        assert_eq!(cartridge.read(0xA07000), Some(0x84));
    }
}
