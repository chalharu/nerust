...
impl Cpu {
        ...
        match self.micro_state {
            ...
            MicroState::DirectIndexedIndirectPointerHigh {
                ...
                if matches!(op, DirectIndexedIndirectOp::Sta) {
                    self.store_accumulator(bus, address);
                } else {
        ...
        match op {
            ...
            DirectOp::Sbc { wide } => {
                ...
                if wide {
                    ...
                    self.apply_immediate_math(ImmediateMathOp::SbcA, u16::from(low));
                    self.micro_state = MicroState::Fetch;
                }
            }
            ...
            DirectOp::Sta { wide } => {
                self.write_operand_value_zero_bank(bus, base, self.registers.a, wide);
            }
            ...
            DirectOp::Stx { wide } => {
                self.write_operand_value_zero_bank(bus, base, self.registers.x, wide);
            }
            ...
            DirectOp::Sty { wide } => {
                self.write_operand_value_zero_bank(bus, base, self.registers.y, wide);
            }
    ...
    fn apply_direct_indexed_indirect(&mut self, op: DirectIndexedIndirectOp, value: u16) {
        match op {
            ...
            DirectIndexedIndirectOp::SbcA => {
                self.apply_immediate_math(ImmediateMathOp::SbcA, value)
            }
            ...
            DirectIndexedIndirectOp::Sta => unreachable!("STA handled during address resolution"),
        }
    }
    ...
    fn execute_direct_indirect(&mut self, bus: &mut dyn CpuBus, op: DirectIndirectOp) {
        ...
        if matches!(op, DirectIndirectOp::Sta) {
            self.store_accumulator(bus, full);
        } else {
    ...
    fn apply_direct_indirect(&mut self, op: DirectIndirectOp, value: u16) {
        match op {
            ...
            DirectIndirectOp::SbcA => self.apply_immediate_math(ImmediateMathOp::SbcA, value),
            ...
            DirectIndirectOp::Sta => unreachable!("STA handled during address resolution"),
        }
    }
    ...
    fn execute_direct_indirect_long(&mut self, bus: &mut dyn CpuBus, op: DirectIndirectLongOp) {
        ...
        self.burn_internal_cycles(bus, extra_cycles);
        if matches!(op, DirectIndirectLongOp::Sta) {
            self.store_accumulator(bus, full);
        } else {
            let value = self.read_operand_value(bus, full, !self.accumulator_is_8bit());
    ...
    fn apply_direct_indirect_long(&mut self, op: DirectIndirectLongOp, value: u16) {
        match op {
            ...
            DirectIndirectLongOp::SbcA => self.apply_immediate_math(ImmediateMathOp::SbcA, value),
            ...
            DirectIndirectLongOp::Sta => unreachable!("STA handled during address resolution"),
        }
    }
    ...
    fn execute_direct_indirect_indexed_y(
        ...
        let (full, extra_cycles) = match op {
            ...
            | DirectIndirectIndexedYOp::SbcA
            | DirectIndirectIndexedYOp::CmpA
            | DirectIndirectIndexedYOp::Lda
            | DirectIndirectIndexedYOp::Sta => {
                let target = self.read_zero_bank_u16(bus, pointer_addr);
                (
            ...
            | DirectIndirectIndexedYOp::SbcALong
            | DirectIndirectIndexedYOp::CmpALong
            | DirectIndirectIndexedYOp::LdaLong
            | DirectIndirectIndexedYOp::StaLong => (
        ...
        if matches!(
            op,
            DirectIndirectIndexedYOp::Sta | DirectIndirectIndexedYOp::StaLong
        ) {
            self.store_accumulator(bus, full);
        } else {
            let value = self.read_operand_value(bus, full, !self.accumulator_is_8bit());
        ...
        match op {
            ...
            | DirectIndirectIndexedYOp::CmpALong => {
                let math_op = match op {
                    ...
                    DirectIndirectIndexedYOp::SbcA | DirectIndirectIndexedYOp::SbcALong => {
                        ImmediateMathOp::SbcA
                    }
                    ...
                    DirectIndirectIndexedYOp::Sta | DirectIndirectIndexedYOp::StaLong => {
                        unreachable!("STA handled during address resolution")
                    }
                };
    ...
    fn apply_stack_relative(&mut self, op: StackRelativeOp, value: u16) {
        match op {
            ...
            StackRelativeOp::SbcA => self.apply_immediate_math(ImmediateMathOp::SbcA, value),
            ...
            StackRelativeOp::Sta => unreachable!("STA handled in execute_stack_relative"),
        }
    }
    ...
    fn apply_stack_relative_indirect_indexed_y(
        ...
        match op {
            ...
            StackRelativeIndirectIndexedYOp::SbcA => {
                self.apply_immediate_math(ImmediateMathOp::SbcA, value)
            }
            ...
            StackRelativeIndirectIndexedYOp::Sta => {
                unreachable!("STA handled during address resolution")
            }
        }
    }
    ...
    fn execute_stack_relative(&mut self, bus: &mut dyn CpuBus, op: StackRelativeOp) {
        ...
        if matches!(op, StackRelativeOp::Sta) {
            self.write_operand_value_zero_bank(
                bus,
                address,
                self.registers.a,
                !self.accumulator_is_8bit(),
            );
        } else {
            let low = bus.read(u32::from(address));
            if self.accumulator_is_8bit() {
    ...
    fn execute_stack_relative_indirect_indexed_y(
        ...
        if matches!(op, StackRelativeIndirectIndexedYOp::Sta) {
            self.store_accumulator(bus, full);
        } else {
            let value = self.read_operand_value(bus, full, !self.accumulator_is_8bit());
    ...
    fn execute_absolute_long_bank(
        ...
        match op {
            ...
            AbsoluteLongOp::SbcIndexedX { wide } => {
                let indexed = full.wrapping_add(u32::from(self.registers.x));
                let low = bus.read(indexed);
                if wide {
                    self.micro_state = MicroState::AbsoluteLongMathHigh {
            ...
            AbsoluteLongOp::Sta { wide } => {
                self.write_operand_value(bus, full, self.registers.a, wide);
            }
            AbsoluteLongOp::StaIndexedX { wide } => {
                self.write_operand_value(
                    bus,
                    full.wrapping_add(u32::from(self.registers.x)),
                    self.registers.a,
                    wide,
                );
            }
    ...
    fn decode_opcode(&mut self, opcode: u8) -> MicroState {
        match opcode {
            ...
            0xE9 => MicroState::ImmediateMathLow(ImmediateMathOp::SbcA),
            ...
            0xE5 => MicroState::Direct(DirectOp::Sbc {
                wide: !self.accumulator_is_8bit(),
            }),
            ...
            0x95 => MicroState::Direct(DirectOp::StaIndexedX {
                wide: !self.accumulator_is_8bit(),
            }),
            0x96 => MicroState::Direct(DirectOp::StxIndexedY {
                wide: !self.index_is_8bit(),
            }),
            0xF5 => MicroState::Direct(DirectOp::SbcIndexedX {
                wide: !self.accumulator_is_8bit(),
            }),
            ...
            0x81 => MicroState::DirectIndexedIndirect(DirectIndexedIndirectOp::Sta),
            0xE1 => MicroState::DirectIndexedIndirect(DirectIndexedIndirectOp::SbcA),
            ...
            0x83 => MicroState::StackRelative(StackRelativeOp::Sta),
            0xE3 => MicroState::StackRelative(StackRelativeOp::SbcA),
            ...
            0x87 => MicroState::DirectIndirectLong(DirectIndirectLongOp::Sta),
            0xE7 => MicroState::DirectIndirectLong(DirectIndirectLongOp::SbcA),
            ...
            0x91 => MicroState::DirectIndirectIndexedY(DirectIndirectIndexedYOp::Sta),
            0xF1 => MicroState::DirectIndirectIndexedY(DirectIndirectIndexedYOp::SbcA),
            0xB1 => MicroState::DirectIndirectIndexedY(DirectIndirectIndexedYOp::Lda),
            0x92 => MicroState::DirectIndirect(DirectIndirectOp::Sta),
            ...
            0xF2 => MicroState::DirectIndirect(DirectIndirectOp::SbcA),
            ...
            0x93 => MicroState::StackRelativeIndirectIndexedY(StackRelativeIndirectIndexedYOp::Sta),
            0xF3 => {
                MicroState::StackRelativeIndirectIndexedY(StackRelativeIndirectIndexedYOp::SbcA)
            }
            ...
            0x97 => MicroState::DirectIndirectIndexedY(DirectIndirectIndexedYOp::StaLong),
            0xF7 => MicroState::DirectIndirectIndexedY(DirectIndirectIndexedYOp::SbcALong),
            ...
            0x99 => MicroState::AbsoluteLow(AbsoluteOp::StaIndexedY {
                wide: !self.accumulator_is_8bit(),
            }),
            0xF9 => MicroState::AbsoluteLow(AbsoluteOp::SbcIndexedY {
                wide: !self.accumulator_is_8bit(),
            }),
            ...
            0xED => MicroState::AbsoluteLow(AbsoluteOp::Sbc {
                wide: !self.accumulator_is_8bit(),
            }),
            ...
            0x9D => MicroState::AbsoluteLow(AbsoluteOp::StaIndexedX {
                wide: !self.accumulator_is_8bit(),
            }),
            0xFD => MicroState::AbsoluteLow(AbsoluteOp::SbcIndexedX {
                wide: !self.accumulator_is_8bit(),
            }),
            ...
            0x8E => MicroState::AbsoluteLow(AbsoluteOp::Stx {
                wide: !self.index_is_8bit(),
            }),
            ...
            0xEF => MicroState::AbsoluteLongLow(AbsoluteLongOp::Sbc {
                wide: !self.accumulator_is_8bit(),
            }),
            ...
            0x9F => MicroState::AbsoluteLongLow(AbsoluteLongOp::StaIndexedX {
                wide: !self.accumulator_is_8bit(),
            }),
            0xFF => MicroState::AbsoluteLongLow(AbsoluteLongOp::SbcIndexedX {
                wide: !self.accumulator_is_8bit(),
            }),
    ...
    fn write_operand_value(&mut self, bus: &mut dyn CpuBus, address: u32, value: u16, wide: bool) {
        bus.write(address, value as u8);
        if wide {
    ...
    fn write_operand_value_zero_bank(
        &mut self,
        bus: &mut dyn CpuBus,
        address: u16,
        value: u16,
        ...
        if wide {
            self.micro_state = MicroState::WriteHigh {
                address: u32::from(address.wrapping_add(1)),
                value: (value >> 8) as u8,
            };
        } else {
            self.micro_state = MicroState::Fetch;
    ...
    fn store_accumulator(&mut self, bus: &mut dyn CpuBus, address: u32) {
        self.write_operand_value(bus, address, self.registers.a, !self.accumulator_is_8bit());
    }
...
mod tests {
    ...
    fn sbc_immediate_16bit_sets_carry_and_zero() {
        ...
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x20, 0xA9, 0x90, 0x90, 0xE9, 0x90, 0x90, 0xDB,
            ],
        );
    ...
    #[test]
    fn sbc_immediate_16bit_sets_overflow_and_negative() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0x18, 0xC2, 0x20, 0xA9, 0x00, 0x70, 0xE9, 0x00, 0xA0, 0xDB,
            ],
        );

        run_until_stopped(&mut cpu, &mut system, 64);

        assert_eq!(cpu.registers().a(), 0xCFFF);
        assert!(!cpu.registers().status().contains(CpuStatus::CARRY));
        assert!(!cpu.registers().status().contains(CpuStatus::ZERO));
        assert!(cpu.registers().status().contains(CpuStatus::NEGATIVE));
        assert!(cpu.registers().status().contains(CpuStatus::OVERFLOW));
    ...
    #[test]
    fn sbc_immediate_8bit_preserves_high_byte() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x20, 0xA9, 0x90, 0xCC, 0xE2, 0x20, 0xE9, 0x90, 0xDB,
            ],
        );
    ...
    #[test]
    fn sbc_direct_indexed_x_uses_direct_page_wrap_and_index() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x000134, &[0x90, 0x90]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0xFF, 0xFF, 0x5B, 0xA9, 0x90, 0x90, 0xA2, 0x33, 0x01,
                0xF5, 0x02, 0xDB,
            ],
        );
    ...
    #[test]
    fn sbc_direct_indexed_indirect_uses_direct_page_and_data_bank() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x00FFA0, &[0x12, 0x12]);
        system.load(0x7F1212, &[0x90, 0x90]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0x7F, 0x00, 0xE2, 0x20, 0x48, 0xAB, 0xC2, 0x20, 0xA9,
                0xFF, 0xFF, 0x5B, 0xA9, 0x90, 0x90, 0xA2, 0x91, 0xFF, 0xE1, 0x10, 0xDB,
            ],
        );
    ...
    #[test]
    fn sbc_stack_relative_indirect_indexed_y_uses_stack_pointer_and_offset() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x0001FF, &[0xDC, 0xFE]);
        system.load(0x7F0FDC, &[0x90, 0x90]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA2, 0xEF, 0x01, 0x9A, 0xA9, 0x7E, 0x00, 0xE2, 0x20, 0x48,
                0xAB, 0xC2, 0x20, 0xA9, 0x90, 0x90, 0xA0, 0x00, 0x11, 0xF3, 0x10, 0xDB,
            ],
        );
    ...
    #[test]
    fn sbc_direct_indirect_long_indexed_y_carries_into_bank() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x000033, &[0xDC, 0xFE, 0x7E]);
        system.load(0x7F0FDC, &[0x90, 0x90]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0xFF, 0xFF, 0x5B, 0xA9, 0x90, 0x90, 0xA0, 0x00, 0x11,
                0xF7, 0x34, 0xDB,
            ],
        );
    ...
    #[test]
    fn sbc_absolute_and_absolute_long_read_memory_operands() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x000033, &[0x90, 0x90]);
        system.load(0x7EFFFF, &[0x90, 0x90]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x20, 0xA9, 0x90, 0x90, 0xED, 0x33, 0x00, 0xA9, 0x90, 0x90, 0xEF,
                0xFF, 0xFF, 0x7E, 0xDB,
            ],
        );
    ...
    #[test]
    fn sbc_absolute_indexed_y_carries_into_data_bank() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x7F02FF, &[0x90, 0x90]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0x7E, 0x00, 0xE2, 0x20, 0x48, 0xAB, 0xC2, 0x20, 0xA9,
                0x90, 0x90, 0xA0, 0x00, 0x03, 0xF9, 0xFF, 0xFF, 0xDB,
            ],
        );
    ...
    #[test]
    fn sbc_absolute_long_indexed_x_carries_into_bank() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x7F02FF, &[0x90, 0x90]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0x90, 0x90, 0xA2, 0x00, 0x03, 0xFF, 0xFF, 0xFF, 0x7E,
                0xDB,
            ],
        );
    ...
    #[test]
    fn sta_direct_indexed_indirect_uses_direct_page_and_data_bank() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x00FFA0, &[0x12, 0x12]);
        system.load(0x7F1212, &[0x34, 0x12]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0x7F, 0x00, 0xE2, 0x20, 0x48, 0xAB, 0xC2, 0x20, 0xA9,
                0xFF, 0xFF, 0x5B, 0xA9, 0x00, 0x80, 0xA2, 0x91, 0xFF, 0x81, 0x10, 0xDB,
            ],
        );
        ...
        assert_eq!(cpu.registers().db(), 0x7F);
        assert_eq!(cpu.registers().d(), 0xFFFF);
        assert_eq!(system.memory.get(&0x7F1212), Some(&0x00));
        assert_eq!(system.memory.get(&0x7F1213), Some(&0x80));
    }

    #[test]
    fn sta_stack_relative_writes_stack_window() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x000201, &[0x34, 0x12]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA2, 0xEF, 0x01, 0x9A, 0xA9, 0x00, 0x80, 0x83, 0x12, 0xDB,
            ],
        );
        ...
        assert_eq!(cpu.registers().a(), 0x8000);
        assert_eq!(cpu.registers().s(), 0x01EF);
        assert_eq!(system.memory.get(&0x000201), Some(&0x00));
        assert_eq!(system.memory.get(&0x000202), Some(&0x80));
    }

    #[test]
    fn sta_stack_relative_wraps_high_byte_at_zero_bank_boundary() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA2, 0xED, 0xFF, 0x9A, 0xA9, 0x00, 0x80, 0x83, 0x12, 0xDB,
            ],
        );
        ...
        assert_eq!(cpu.registers().a(), 0x8000);
        assert_eq!(cpu.registers().s(), 0xFFED);
        assert_eq!(system.memory.get(&0x00FFFF), Some(&0x00));
        assert_eq!(system.memory.get(&0x000000), Some(&0x80));
    }

    #[test]
    fn sta_direct_writes_wrapped_direct_page() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x000033, &[0x34, 0x12]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0xFF, 0xFF, 0x5B, 0xA9, 0x00, 0x80, 0x85, 0x34, 0xDB,
            ],
        );
    ...
    #[test]
    fn sta_direct_wraps_high_byte_at_zero_bank_boundary() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0xFF, 0xFF, 0x5B, 0xA9, 0x00, 0x80, 0x85, 0x00, 0xDB,
            ],
        );
    ...
    #[test]
    fn sta_direct_indirect_long_writes_full_24bit_pointer() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x000033, &[0x34, 0x12, 0x7F]);
        system.load(0x7F1234, &[0x34, 0x12]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0xFF, 0xFF, 0x5B, 0xA9, 0x00, 0x80, 0x87, 0x34, 0xDB,
            ],
        );
        ...
        assert_eq!(cpu.registers().a(), 0x8000);
        assert_eq!(cpu.registers().d(), 0xFFFF);
        assert_eq!(system.memory.get(&0x7F1234), Some(&0x00));
        assert_eq!(system.memory.get(&0x7F1235), Some(&0x80));
    }

    #[test]
    fn sta_absolute_and_absolute_long_write_expected_bytes() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x7F1234, &[0x34, 0x12]);
        ...
        system.load(
            ...
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0x7F, 0x00, 0xE2, 0x20, 0x48, 0xAB, 0xC2, 0x20, 0xA9,
                0x00, 0x80, 0x8D, 0x34, 0x12, 0x8F, 0xFF, 0xFF, 0x7E, 0xDB,
            ],
        );
    ...
    #[test]
    fn sta_direct_indirect_indexed_y_writes_with_data_bank_offset() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x000033, &[0xDC, 0xFE]);
        system.load(0x7F0FDC, &[0x34, 0x12]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0x7E, 0x00, 0xE2, 0x20, 0x48, 0xAB, 0xC2, 0x20, 0xA9,
                0xFF, 0xFF, 0x5B, 0xA9, 0x00, 0x80, 0xA0, 0x00, 0x11, 0x91, 0x34, 0xDB,
            ],
        );
        ...
        assert_eq!(cpu.registers().y(), 0x1100);
        assert_eq!(cpu.registers().db(), 0x7E);
        assert_eq!(system.memory.get(&0x7F0FDC), Some(&0x00));
        assert_eq!(system.memory.get(&0x7F0FDD), Some(&0x80));
    }

    #[test]
    fn sta_direct_indirect_writes_via_data_bank_pointer() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x000033, &[0x34, 0x12]);
        system.load(0x7F1234, &[0x34, 0x12]);
        system.load(
            ...
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0x7F, 0x00, 0xE2, 0x20, 0x48, 0xAB, 0xC2, 0x20, 0xA9,
                0xFF, 0xFF, 0x5B, 0xA9, 0x00, 0x80, 0x92, 0x34, 0xDB,
            ],
        );
    ...
    #[test]
    fn sta_stack_relative_indirect_indexed_y_writes_using_stack_pointer_and_offset() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x0001FF, &[0xDC, 0xFE]);
        system.load(0x7F0FDC, &[0x34, 0x12]);
        system.load(
            ...
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA2, 0xEF, 0x01, 0x9A, 0xA9, 0x7E, 0x00, 0xE2, 0x20, 0x48,
                0xAB, 0xC2, 0x20, 0xA9, 0x00, 0x80, 0xA0, 0x00, 0x11, 0x93, 0x10, 0xDB,
            ],
        );
    ...
    #[test]
    fn sta_direct_indexed_x_writes_wrapped_direct_page_address() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x000134, &[0x34, 0x12]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0xFF, 0xFF, 0x5B, 0xA9, 0x00, 0x80, 0xA2, 0x33, 0x01,
                0x95, 0x02, 0xDB,
            ],
        );
        ...
        assert_eq!(cpu.registers().d(), 0xFFFF);
        assert_eq!(system.memory.get(&0x000134), Some(&0x00));
        assert_eq!(system.memory.get(&0x000135), Some(&0x80));
    }

    #[test]
    fn sta_direct_indirect_long_indexed_y_carries_into_bank() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x000033, &[0xDC, 0xFE, 0x7E]);
        system.load(0x7F0FDC, &[0x34, 0x12]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0xFF, 0xFF, 0x5B, 0xA9, 0x00, 0x80, 0xA0, 0x00, 0x11,
                0x97, 0x34, 0xDB,
            ],
        );
    ...
    #[test]
    fn sta_absolute_indexed_y_carries_into_data_bank() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        ...
        system.load(
            ...
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0x7E, 0x00, 0xE2, 0x20, 0x48, 0xAB, 0xC2, 0x20, 0xA9,
                0x00, 0x80, 0xA0, 0x00, 0x03, 0x99, 0xFF, 0xFF, 0xDB,
            ],
        );
        ...
        assert_eq!(cpu.registers().y(), 0x0300);
        assert_eq!(system.memory.get(&0x7F02FF), Some(&0x00));
        assert_eq!(system.memory.get(&0x7F0300), Some(&0x80));
    }

    #[test]
    fn sta_absolute_indexed_x_and_long_indexed_x_carry_into_bank() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x7F02FF, &[0x34, 0x12]);
        system.load(0x7F01FF, &[0x78, 0x56]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0x7E, 0x00, 0xE2, 0x20, 0x48, 0xAB, 0xC2, 0x20, 0xA9,
                0x00, 0x80, 0xA2, 0x00, 0x03, 0x9D, 0xFF, 0xFF, 0xA9, 0x34, 0x12, 0x9F, 0xFF, 0xFE,
                0x7E, 0xDB,
            ],
        );
        ...
        assert_eq!(system.memory.get(&0x7F02FF), Some(&0x00));
        assert_eq!(system.memory.get(&0x7F0300), Some(&0x80));
        assert_eq!(system.memory.get(&0x7F01FF), Some(&0x34));
        assert_eq!(system.memory.get(&0x7F0200), Some(&0x12));
    }

    #[test]
    fn stx_direct_indexed_y_writes_wrapped_direct_page_address() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x000134, &[0x34, 0x12]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0xFF, 0xFF, 0x5B, 0xA2, 0x00, 0x80, 0xA0, 0x33, 0x01,
                0x96, 0x02, 0xDB,
            ],
        );
    ...
    #[test]
    fn stx_direct_indexed_y_wraps_high_byte_at_zero_bank_boundary() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0xFF, 0xFF, 0x5B, 0xA2, 0x00, 0x80, 0xA0, 0x00, 0x00,
                0x96, 0x00, 0xDB,
            ],
        );
    ...
    #[test]
    fn stx_absolute_writes_across_bank_boundary() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x7EFFFF, &[0x34, 0x12]);
        system.load(
    ...
    #[test]
    fn sty_direct_indexed_x_writes_wrapped_direct_page_address() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x000134, &[0x34, 0x12]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0xFF, 0xFF, 0x5B, 0xA0, 0x00, 0x80, 0xA2, 0x33, 0x01,
                0x94, 0x02, 0xDB,
            ],
        );
    ...
    #[test]
    fn sty_direct_indexed_x_wraps_high_byte_at_zero_bank_boundary() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0xFF, 0xFF, 0x5B, 0xA0, 0x00, 0x80, 0xA2, 0x00, 0x00,
                0x94, 0x00, 0xDB,
            ],
        );
    ...
    #[test]
    fn sty_absolute_writes_across_bank_boundary() {
        let mut cpu = Cpu::new();
        let mut system = TestBus::with_reset_vector(0x8000);
        system.load(0x7EFFFF, &[0x34, 0x12]);
        system.load(
            0x008000,
            &[
                0x18, 0xFB, 0xC2, 0x30, 0xA9, 0x7E, 0x00, 0xE2, 0x20, 0x48, 0xAB, 0xC2, 0x20, 0xA0,
                0x00, 0x80, 0x8C, 0xFF, 0xFF, 0xDB,
            ],
        );
