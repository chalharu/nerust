...
impl Apu {
    ...
    fn smp_instruction_cycles(&self, opcode: u8) -> u32 {
        match opcode {
            ...
            0xFE => {
                if self.smp_y.wrapping_sub(1) != 0 { 6 } else { 4 }
            }
            0xEF | 0xFF => 2,
            _ => 2,
        }
...
#[cfg(test)]
mod tests {
    use super::{Apu, DSP_KEY_ON, DSP_SOURCE_DIRECTORY, SMP_FLAG_C, SMP_FLAG_N, SMP_FLAG_V, SMP_FLAG_Z};
    use nerust_sound_traits::MixerInput;
    ...
    fn smp_instruction_cycles_match_fullsnes_table() {
        ...
        assert_opcode_cycles_for_all(
            &apu,
            &[0x01, 0x11, 0x21, 0x31, 0x41, 0x51, 0x61, 0x71, 0x81, 0x91, 0xA1, 0xB1, 0xC1,
                0xD1, 0xE1, 0xF1],
            8,
        );
        assert_opcode_cycles_for_all(
        ...
        assert_opcode_cycles_for_all(&apu, &[0x14, 0x15, 0x16, 0x17, 0x34, 0x35, 0x36, 0x37,
            0x54, 0x55, 0x56, 0x57, 0x74, 0x75, 0x76, 0x77, 0x94, 0x95, 0x96, 0x97, 0xB4, 0xB5,
            0xB6, 0xB7, 0xD4, 0xD5, 0xD6, 0xD7, 0xF4, 0xF5, 0xF6, 0xF7], 4);
        assert_opcode_cycles_for_all(&apu, &[0x18, 0x19, 0x38, 0x39, 0x58, 0x59, 0x78, 0x79,
            0x98, 0x99, 0xB8, 0xB9], 5);
        assert_opcode_cycles_for_all(&apu, &[0x1A, 0x3A], 6);
        assert_opcode_cycles_for_all(&apu, &[0x1B, 0x3B, 0x5B, 0x7B], 5);
        assert_opcode_cycles_for_all(&apu, &[0x1C, 0x3C, 0x5C, 0x7C, 0x9C, 0xDC, 0xFC, 0x1D,
            0x3D, 0x5D, 0x7D, 0x9D, 0xBD, 0xDD, 0xFD, 0xBF, 0xCE], 2);
