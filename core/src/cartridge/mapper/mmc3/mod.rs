...
mod tests {
    ...
    #[test]
    fn scheduled_chr_bank_change_is_deferred_until_flush() {
    ...
    #[test]
    fn scheduled_chr_layout_flip_is_deferred_until_flush() {
        let mut mapper = new_mmc3_with_chr_banks(0);
        let mut interrupt = Interrupt::new();
        ...
        Mapper::schedule_register_write(&mut mapper, 0x8000, 0x82, &mut interrupt);
    ...
    #[test]
    fn scheduled_program_ram_protect_write_stays_immediate() {
        let mut mapper = new_mmc3(0);
        let mut interrupt = Interrupt::new();

        assert_eq!(Mapper::read_ram(&mapper, 0x0000), Some(0));

        Mapper::schedule_register_write(&mut mapper, 0xA001, 0x00, &mut interrupt);
