...
pub(crate) trait Mapper: MapperStateDao + CartridgeDataDao {
    ...
    ///
    /// Most mappers apply the write immediately. Mappers with CPU/PPU phase-sensitive
    /// behavior may defer the write until `flush_deferred_register_writes` is called.
